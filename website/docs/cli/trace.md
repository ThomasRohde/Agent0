...
When `a0 trace` exits with code 4, it emits a diagnostic:

- `E_IO` -- trace file could not be read (missing path, permissions, etc.)
- `E_TRACE` -- file was readable but had invalid trace content (for example, no valid events or mixed `runId` values)
